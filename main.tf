...
module "project-svc-gke" {
  ...
  name            = "gke"
  #services        = var.project_services
  services        = concat(var.project_services, ["apigee.googleapis.com",  "cloudkms.googleapis.com", "servicenetworking.googleapis.com", "anthos.googleapis.com", "mesh.googleapis.com", "gkehub.googleapis.com", "meshca.googleapis.com", "meshtelemetry.googleapis.com"])
  shared_vpc_service_config = {
    host_project = module.project-host.project_id
    service_identity_iam = {
      "roles/container.hostServiceAgentUser" = ["container-engine"]
      "roles/compute.networkUser"            = ["container-engine"]
      #"roles/gkehub.admin"                   =  ["container-engine"]
    }
  }
  iam = merge(
    {
    ...
    ? {
      ...
      #"roles/gkehub.admin"            = [module.cluster-1-nodepool-1.0.service_account_iam_email]
    }
    : {}
  )
}
...
module "cluster-1" {
  ...
  #cluster_resource_labels = { "mesh_id" : "proj-${module.project-svc-gke.number}" }
...
provider "kubernetes" {
  host                   = "https://${module.cluster-1.0.endpoint}"
  token                  = data.google_client_config.default.access_token
  cluster_ca_certificate = base64decode(module.cluster-1.0.ca_certificate)
}
